...
def make_csr(name_workload, shape):
...
def load_csr(name_workload, shape_train, shape_test):
	root_dir    = os.environ['DATASETSROOT']
	dataset_dir = os.path.join(root_dir, 'workloads', name_workload, 'dataset')
	x_train = make_csr(os.path.join(dataset_dir, '{}_x_train'.format(name_workload)), shape_train)
	x_test = make_csr(os.path.join(dataset_dir, '{}_x_test'.format(name_workload)), shape_test)
	y_train = np.load(os.path.join(dataset_dir, '{}_y_train.npy'.format(name_workload)))
	y_test = np.load(os.path.join(dataset_dir, '{}_y_test.npy'.format(name_workload)))
	return x_train, x_test, y_train, y_test
...
#     times_worloads.append(run_svm_workload(name_workload, x_train, x_test, y_train, y_test, C=1, kernel='rbf'))

# name_workload = 'ijcnn'
# if arg_name_workload in [name_workload, 'all']:
#     # Predict connections in an online social network
#     x_train, x_test, y_train, y_test = load_data(name_workload)
#     times_worloads.append(run_svm_workload(name_workload, x_train, x_test, y_train, y_test, C=1, kernel='linear'))
...
if arg_name_workload in [name_workload, 'all']:
    # Predict next moves in game of connect-4.
    x_train, x_test, y_train, y_test = load_csr(name_workload, (17935, 62061), (1993,  62061))
    times_worloads.append(run_svm_workload(name_workload, x_train, x_test, y_train, y_test, C=0.1, kernel='rbf'))    

name_workload = 'mnist'
if arg_name_workload in [name_workload, 'all']:
    # Predict next moves in game of connect-4.
    x_train, x_test, y_train, y_test = load_csr(name_workload, (60000, 780), (10000 , 780))
    times_worloads.append(run_svm_workload(name_workload, x_train, x_test, y_train, y_test, C=1, kernel='rbf'))  

name_workload = 'rcv1'
if arg_name_workload in [name_workload, 'all']:
    # Predict next moves in game of connect-4.
    x_train, x_test, y_train, y_test = load_csr(name_workload, (20242 , 47236), (677399 , 47236))
    times_worloads.append(run_svm_workload(name_workload, x_train, x_test, y_train, y_test, C=0.01, kernel='rbf'))      
