...
def run_svm_proba_workload(workload_name, x_train, x_test, y_train, y_test, C=1.0, kernel='linear'):
	...
	time_train_run = t1 - t0

	t0 = timeit.default_timer()
	y_pred_train = clf.predict_proba(x_train)
	t1 = timeit.default_timer()
	time_predict_run = t1 - t0

	acc_train = log_loss(y_train, y_pred_train)

	y_pred = clf.predict_proba(x_test)
	acc_test = log_loss(y_test, y_pred)
	print('Workload {} {}x{} running fit: {:.2f} (sec), predict_proba: {:.2f} (sec) have log_loss train: {:.3f}, test: {:.3f}'.format(workload_name, 
		x_train.shape[0], x_train.shape[1], time_train_run, time_predict_run, acc_train, acc_test))
	return time_train_run
...
if arg_name_workload in [name_workload, 'all']:
    ...
    times_worloads.append(run_svm_proba_workload(name_workload, x_train, x_test, y_train, y_test, C=500, kernel='rbf'))

name_workload = 'ijcnn'
if arg_name_workload in [name_workload, 'all']:
    # Predict connections in an online social network
    x_train, x_test, y_train, y_test = load_data(name_workload)
    times_worloads.append(run_svm_proba_workload(name_workload, x_train, x_test, y_train, y_test, C=1000, kernel='linear'))
...
if arg_name_workload in [name_workload, 'all']:
    ...
    name_workload = 'sensit'
    x_train, x_test, y_train, y_test = load_data(name_workload)
    times_worloads.append(run_svm_proba_workload(name_workload, x_train, x_test, y_train, y_test, C=500, kernel='linear'))

name_workload = 'connect'
if arg_name_workload in [name_workload, 'all']:
    # Predict next moves in game of connect-4.
    x_train, x_test, y_train, y_test = load_data(name_workload)
    times_worloads.append(run_svm_proba_workload(name_workload, x_train, x_test, y_train, y_test, C=100, kernel='linear'))
