...
if arg_name_workload in [name_workload, 'all']:
    ...
    times_worloads.append(run_svm_workload(name_workload, x_train, x_test, y_train, y_test, C=500, kernel='rbf'))

name_workload = 'ijcnn'
if arg_name_workload in [name_workload, 'all']:
    # Predict connections in an online social network
    x_train, x_test, y_train, y_test = load_data(name_workload)
    times_worloads.append(run_svm_workload(name_workload, x_train, x_test, y_train, y_test, C=1000, kernel='linear'))
...
if arg_name_workload in [name_workload, 'all']:
    ...
    name_workload = 'sensit'
    x_train, x_test, y_train, y_test = load_data(name_workload)
    times_worloads.append(run_svm_workload(name_workload, x_train, x_test, y_train, y_test, C=500, kernel='linear'))

name_workload = 'connect'
if arg_name_workload in [name_workload, 'all']:
    # Predict next moves in game of connect-4.
    x_train, x_test, y_train, y_test = load_data(name_workload)
    times_worloads.append(run_svm_workload(name_workload, x_train, x_test, y_train, y_test, C=100, kernel='linear'))
